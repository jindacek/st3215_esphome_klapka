...
CONF_STARTUP_SYNC_DELAY_MS = "startup_sync_delay_ms"
CONF_STARTUP_VALID_READS = "startup_valid_reads"
CONF_STARTUP_MAX_DELTA_TURNS = "startup_max_delta_turns"
...
_SERVO_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_STARTUP_SYNC_DELAY_MS, default=3000): cv.positive_int,
        cv.Optional(CONF_STARTUP_VALID_READS, default=5): cv.int_range(min=1, max=20),
        cv.Optional(CONF_STARTUP_MAX_DELTA_TURNS, default=0.003): cv.float_range(min=0.0005, max=0.050),
...
async def to_code(config):
    for conf in config:
        ...
        cg.add(var.set_startup_sync_delay_ms(conf[CONF_STARTUP_SYNC_DELAY_MS]))
        cg.add(var.set_startup_valid_reads(conf[CONF_STARTUP_VALID_READS]))
        cg.add(var.set_startup_max_delta_turns(conf[CONF_STARTUP_MAX_DELTA_TURNS]))
