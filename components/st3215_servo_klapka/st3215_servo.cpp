...
void St3215Servo::update() {
  ...
  if (!startup_synced_) {
    ...
    if (has_zero_) {
      // U klapky je rozsah menší než jedna otáčka. Nejbezpečnější je navázat aktuální raw
      // na nejbližší celou otáčku k uložené nule, ne na poslední uloženou pozici.
      turns_base_ = static_cast<int32_t>(std::round(zero_offset_ - frac));
      turns_unwrapped_ = turns_base_ + frac;
    } else if (has_stored_turns_) {
    ...
    ESP_LOGI(TAG, "Startup sync OK: raw=%u logical=%u frac=%.3f base=%ld current=%.3f zero=%.3f total=%.3f reads=%u",
             raw, logical_raw, frac, (long) turns_base_, turns_unwrapped_, zero_offset_,
             fabsf(turns_unwrapped_ - zero_offset_), startup_stable_count_);
    return;
  }

  // První krok po startu / po případném recovery, pokud už startup sync proběhl
  if (!have_last_) {
    last_raw_ = logical_raw;
    float frac = logical_raw / RAW_PER_TURN;
    turns_base_ = has_zero_ ? static_cast<int32_t>(std::round(zero_offset_ - frac)) : 0;
    turns_unwrapped_ = turns_base_ + frac;
    have_last_ = true;
    ESP_LOGI(TAG, "Encoder sync: current=%.3f zero=%.3f raw=%u", turns_unwrapped_, zero_offset_, raw);
    return;
  }
