...
static constexpr float DECEL_ZONE    = 0.08f;  // základ brzdné zóny pro krátkou klapku (otáčky)
...
void St3215Servo::setup() {
  ...
  // v0.7: po bootu je poslední uložená poloha zdroj pravdy.
  // První čtení enkodéru nesmí přepsat current pozici, protože ST3215/Waveshare
  // po studeném startu občas vrátí nepatrně posunutou hodnotu a klapka pak
  // startuje jako 8–10 % místo 0 %. Enkodér dále používáme inkrementálně
  // pro změny polohy během pohybu.
  if (loaded && has_stored_turns_) {
    turns_unwrapped_ = stored_turns_;
    ESP_LOGI(TAG, "BOOT RESTORE: using saved position pos=%.3f (zero=%.3f, max=%.3f)",
  ...
  startup_sync_started_ms_ = millis();
  startup_synced_ = false;
  startup_stable_count_ = 0;
  startup_have_raw_ = false;
  have_last_ = false;
  ESP_LOGI(TAG, "Startup sync: delay=%ums valid_reads=%u max_delta=%.4f turns",
           startup_sync_delay_ms_, startup_valid_reads_, startup_max_delta_turns_);
}
...
void St3215Servo::update() {
  ...
  // ===== STARTUP SYNC =====
  // Po tvrdém zapnutí napájení nebereme první čtení jako definitivní.
  // Počkáme na několik stabilních raw pozic a pak se navážeme na uloženou nulu BOX1.
  if (!startup_synced_) {
    uint32_t now_sync = millis();

    if ((now_sync - startup_sync_started_ms_) < startup_sync_delay_ms_) {
      return;
    }

    bool stable = true;
    if (startup_have_raw_) {
      int d = (int) logical_raw - (int) startup_last_raw_;
      if (d > 2048) d -= 4096;
      if (d < -2048) d += 4096;
      stable = fabsf((float) d / RAW_PER_TURN) <= startup_max_delta_turns_;
    }

    startup_last_raw_ = logical_raw;
    startup_have_raw_ = true;

    if (stable) {
      if (startup_stable_count_ < 255) startup_stable_count_++;
    } else {
      startup_stable_count_ = 1;
    }

    if (startup_stable_count_ < startup_valid_reads_) {
      return;
    }

    float frac = logical_raw / RAW_PER_TURN;

    // v0.7: navázání enkodéru na poslední uloženou pozici.
    // Neuděláme current = base + frac, protože tím by první studené čtení mohlo
    // přepsat uloženou nulu. Enkodér od této chvíle slouží jako inkrementální
    // měření změny: další update přičte pouze rozdíl raw hodnot.
    float before = turns_unwrapped_;
    ...
    have_last_ = true;
    startup_synced_ = true;

    ESP_LOGI(TAG, "Startup sync OK v0.7: raw=%u logical=%u frac=%.3f saved_before=%.3f current=%.3f zero=%.3f total=%.3f reads=%u",
             raw, logical_raw, frac, before, turns_unwrapped_, zero_offset_,
             fabsf(turns_unwrapped_ - zero_offset_), startup_stable_count_);
    return;
  }

  // První krok po případném recovery, pokud už startup sync proběhl
  if (!have_last_) {
    last_raw_ = logical_raw;
    have_last_ = true;
    ESP_LOGI(TAG, "Encoder sync: keeping current=%.3f zero=%.3f raw=%u", turns_unwrapped_, zero_offset_, raw);
    return;
  }

  int diff = (int) logical_raw - (int) last_raw_;

  // normalizace přes přetečení 0/4095
  ...
  // v0.7: pozici vedeme inkrementálně od uložené pos, ne absolutně z raw.
  // Tím se po studeném startu zachová naposled uložených 0 %.
  turns_unwrapped_ += ((float) diff / RAW_PER_TURN);
  ...
  if (percent_sensor_ && has_zero_ && has_max_) {
    ...
    if (position_mode_) {
      if (position_turns_mode_) {
        float err = fabsf(total - target_turns_);
        if (err < STOP_EPS) {
          ESP_LOGI(TAG, "TARGET ANGLE REACHED total=%.3f target=%.3f err=%.3f", total, target_turns_, err);
          stop();
          position_mode_ = false;
          position_turns_mode_ = false;
        }
      } else {
  ...
  if (now - last_ramp_update_ >= RAMP_DT_MS) {
    ...
    if (has_zero_ && has_max_) {
      ...
      // v0.4: brzdná zóna je vztažená k reálnému rozsahu klapky, ne k roletě.
      // Pro klapku s rozsahem ~0.12–0.25 otáčky by původní 0.90 znamenalo,
      // že servo bylo pořád v brzdění a změny rampy/rychlosti skoro nebyly poznat.
      float base_decel_zone = DECEL_ZONE;
      if (has_max_ && max_turns_ > 0.0f) {
        base_decel_zone = max_turns_ * 0.45f;
        if (base_decel_zone < 0.015f) base_decel_zone = 0.015f;
        if (base_decel_zone > 0.12f)  base_decel_zone = 0.12f;
      }
      float decel_zone = base_decel_zone * ramp_factor_;
...
void St3215Servo::safety_stop_(const std::string &reason) {
  ...
  // v0.7: bezpečnostní STOP NESMÍ uložit chybnou pozici do flash.
  // Při studeném startu se může první pohyb vyhodnotit jako stall a starší verze
  // si tím přepsala poslední dobrou BOX1 pozici na např. 8–10 %.
  suppress_next_stop_save_ = true;
...
void St3215Servo::move_to_percent(float pct) {
  if (!startup_synced_) {
    ESP_LOGW(TAG, "MOVE PERCENT ignored – startup sync not ready");
    return;
  }
  if (!has_zero_ || !has_max_)
    return;
...
void St3215Servo::move_to_angle(float degrees) {
  if (!startup_synced_) {
    ESP_LOGW(TAG, "MOVE ANGLE ignored – startup sync not ready");
    return;
  }
  if (!has_zero_) {
    ESP_LOGW(TAG, "MOVE ANGLE ignored – missing BOX1/0° calibration");
    return;
  }
  ...
  // v0.3: čistě úhlová logika bez procentového mezikroku.
  // Cíl = uložená nula + požadovaný úhel převedený na otáčky.
  float target = degrees / 360.0f;
  if (target < 0.0f) target = 0.0f;

  // Soft rozsah necháme minimálně na max_angle_, aby ochrany pořád dávaly smysl.
  max_turns_ = max_angle_ / 360.0f;
  if (max_turns_ < min_calib_span_turns_)
    max_turns_ = min_calib_span_turns_;
  has_max_ = true;

  float current = fabsf(turns_unwrapped_ - zero_offset_);
  target_turns_ = target;
  target_percent_ = (max_turns_ > 0.0f) ? (target / max_turns_) * 100.0f : 0.0f;
  position_mode_ = true;
  position_turns_mode_ = true;

  ESP_LOGI(TAG, "MOVE ANGLE: requested=%.1f°, zero=%.3f, current=%.3f, target_turns=%.3f, max_turns=%.3f",
           degrees, zero_offset_, current, target_turns_, max_turns_);

  if (fabsf(current - target_turns_) < STOP_EPS) {
    stop();
    position_mode_ = false;
    position_turns_mode_ = false;
    return;
...
void St3215Servo::stop() {
  ...
  // v0.7: neukládat při safety_stop_(), aby se do flash nezapsala chybná pozice.
  if (has_zero_ && has_max_ && !suppress_next_stop_save_) {
    save_calibration_();
  } else if (suppress_next_stop_save_) {
    ESP_LOGW(TAG, "STOP: position save suppressed after SAFETY STOP");
    suppress_next_stop_save_ = false;
  }
...
void St3215Servo::rotate(bool cw, int speed) {
  if (!startup_synced_) {
    ESP_LOGW(TAG, "ROTATE ignored – startup sync not ready");
    return;
  }
