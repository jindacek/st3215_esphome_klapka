...
void St3215Servo::setup() {
  ...
  if (!has_zero_ || !has_max_) {
    ...
    ESP_LOGI(TAG, loaded
                 ? "Stored calibration incomplete – Nutná kalibrace klapky"
                 : "Nutná kalibrace klapky");
  } else {
    update_calib_state_(CALIB_DONE);
    ESP_LOGI(TAG, "Klapka připravena (max_turns=%.2f)", max_turns_);
  }
}
...
void St3215Servo::set_max() {
  ...
  if (span < min_calib_span_turns_) return;
  max_turns_ = span;
  has_max_ = true;
}
